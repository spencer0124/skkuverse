...
- ⚠️ **Freshness is deliberately not blocking.** A consumer being behind is a working system, not a broken one, and it is not fixable in the branch that would go red. The governing rule for this whole system: *a red check the author cannot fix in the current branch is worse than no check*, because it teaches people to merge anyway.
